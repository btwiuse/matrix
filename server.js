...
for (let i = 0; i < argv.length; i++) {
  ...
  const next = argv[i + 1];
  if (next && !next.startsWith('--')) { args[name] = next; i++; } else { args[name] = true; }
}
...
const marker = `\n<!-- html-inject:${crypto.createHash('sha256').update(injection).digest('hex').slice(0, 12)} -->`;

function injectInto(html) {
  if (html.includes(marker.trim())) return html;   // 已注入,幂等
  const body = html.lastIndexOf('</body>');        // 首选 </body> 前
  if (body !== -1) return html.slice(0, body) + marker + '\n' + injection + '\n' + html.slice(body);
  const doc = html.lastIndexOf('</html>');         // 兜底 </html> 前
  if (doc !== -1) return html.slice(0, doc) + marker + '\n' + injection + '\n' + html.slice(doc);
  return html + marker + '\n' + injection;         // 最后兜底:追加末尾
}
...
const server = http.createServer((req, res) => {
    ...
    pres.on('end', () => {
      ...
      if (ct.includes('text/html')) {
        ...
        if (injected !== text) {
          ...
          if (verbose) console.log(`[inject] ${req.method} ${req.url} (${raw.length} -> ${outBody.length} 字节)`);
        } else if (verbose) {
          console.log(`[skip]   ${req.method} ${req.url} 已注入,跳过`);
        }
      } else if (verbose) {
        console.log(`[pass]   ${req.method} ${req.url} 非 text/html (${ct || '无 Content-Type'})`);
      }
  ...
  preq.on('error', (e) => { res.writeHead(502); res.end('Bad gateway: ' + e.message); });
  req.pipe(preq);
});
