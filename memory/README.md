...
| [`04-repo-layout.md`](04-repo-layout.md) | 本仓库布局:htmlinject(根包) + matrix(子包) + rewrite + cmd/matrix 单一入口;迁移历史与命名约定 | 新增文件/命令、理解包边界、提交 |
...
- 复刻 server 支持 `-mode auto|proxy|mock`,唯一 transport 是 streamable HTTP(默认 :8080/$PORT),`--data-dir` 开启同一进程的站点托管(Host 路由),README 见 `matrix/README.md`
- 测试: `go test ./...`(全绿);22 个工具全部有调用级测试覆盖(详见 05)
